...
public:
    ...
    DoublyNode *nextN; // address element of next node
    DoublyNode *prevN; // addesss element of prev node
};

// function which creates n nodes given size n and array 'arr' specifying n data elements

DoublyNode *create_nNodes(int n, int arr[])
{

    DoublyNode *head = NULL, *temp = NULL, *newnode; // creating pointers for accessing the ll

    for (int i = 0; i < n; i++)
    {
        newnode = new DoublyNode(); // allot memory to newnode
